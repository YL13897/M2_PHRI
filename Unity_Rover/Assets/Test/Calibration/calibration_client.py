...
class ForceBar(QWidget):
    ...
    def set_values(self, force, target):
        self.force = force
        self.target = target
        self.update()
    ...
    def paintEvent(self, _event):
        ...
        painter.drawText(10, y - 10, "Force Target")
...
class CalibrationApp(QWidget):
    ...
    def show_result(self, text, error=False):
        ...
        style = "color: #c62828; font-weight: bold;" if error else ""
    ...
    def finish_step(self, key, samples):
        ...
        samples = [
            ...
            for sample in trim_samples(repeat_samples)
        ]
        n = len(repeats)
        if key == "emg_rest":
        ...
        self.show_result(self.result_text)
        self.append_log(f"{self.result_text} | n={n}")

    def fit_stiffness(self):
        try:
            result = fit_emg_force(self.profile)
        except ValueError as ex:
            self.show_result(str(ex), error=True)
            self.append_log(f"Fit failed: {self.result_text}")
            return

        weights, force_bias, stiffness_scale, fit_trials, fit_slots = result
        ...
        emg_slots = self.last_sample.get("emgSlots", [])
        fit_channels = [emg_slots[slot] for slot in fit_slots if slot < len(emg_slots)]
        channel_text = ", ".join(f"Ch{channel}" for channel in fit_channels)
        self.profile.note = f"Stiffness fitted using {channel_text}."
        self.show_result(
            f"stiffness_scale = {stiffness_scale:.3f} | "
            f"ridge_alpha = {self.profile.ridge_alpha:g} | "
            f"fit channels = {channel_text}"
        )
    ...
    def update_ui(self):
        ...
        emg_slots = sample.get("emgSlots", [])
        slot_values = slot_emg_values(sample)
        selected_slots = selected_emg_slots(sample)
        ...
        self.emg_label.setText(
            ...
            + (
                ", ".join(
                    f"Ch{emg_slots[slot]}: "
                    + (
                        # Use fixed-point format for normal-sized values
                        f"{slot_values[slot]:.6f}"
                        if abs(slot_values[slot]) >= 1e-4
                        # Use scientific notation for very small values
                        else f"{slot_values[slot]:.3e}"
                    )
                    for slot in selected_slots
                )
                if selected_slots
                else "--"  # Show "--" if no slots are selected
            )
        )
        if sample.get("hasFsr", False):
            self.grasp_label.setText(
                f"Grasp: {float(sample.get('graspForceN', 0.0)):.3f} N"
            )
        else:
        ...
        self.force_bar.set_values(force_norm(sample, self.profile), target)
